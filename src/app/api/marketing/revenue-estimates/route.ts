...
const estimateSchema = z
  ...
  .superRefine((value, context) => {
    if (
      Math.abs(
        value.airbnbMixRate + value.bookingMixRate + value.directMixRate - 1,
      ) > 0.0001
    ) {
      context.addIssue({
    ...
    if (
      ...
      (value.adrPerNight === null || value.occupancyRate === null)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["adrPerNight"],
        message: "Inserisci ADR e tasso di occupazione.",
      });
    }
    if (
      value.calculationMode === "annual_revenue" &&
      value.annualGrossRevenueInput === null
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["annualGrossRevenueInput"],
        message: "Inserisci il fatturato lordo annuo.",
      });
    }
  });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const logoEntries = await Promise.all(
      [
        ...new Set(
          estimates.data.map((estimate) => estimate.logo_path).filter(Boolean),
        ),
      ].map(async (path) => {
    ...
    return NextResponse.json({
      ...
      logoUrls: Object.fromEntries(logoEntries),
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  return saveEstimate(request, false);
}
export async function PATCH(request: NextRequest) {
  return saveEstimate(request, true);
}
...
async function saveEstimate(request: NextRequest, isUpdate: boolean) {
  try {
    ...
    const values = {
      ...
      adr_per_night:
        payload.calculationMode === "adr_occupancy"
          ? payload.adrPerNight
          : null,
      occupancy_rate:
        payload.calculationMode === "adr_occupancy"
      ...
      annual_gross_revenue_input:
        payload.calculationMode === "annual_revenue"
          ? payload.annualGrossRevenueInput
          : null,
    ...
    await writeAdminAuditLog({
      ...
      after: {
        ...
        grossAnnualRevenue: data.gross_annual_revenue,
      },
    });
