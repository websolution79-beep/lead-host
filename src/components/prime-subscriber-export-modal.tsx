...
export type PrimeSubscriberExportColumn =
  | "name"
  | "email"
  | "phone"
  | "accountStatus"
...
const exportColumns: Array<{ id: PrimeSubscriberExportColumn; label: string }> = [
  { id: "name", label: "Nome e cognome" },
  { id: "email", label: "Email" },
  { id: "phone", label: "Telefono" },
  { id: "accountStatus", label: "Stato account Lead Host" },
  { id: "primaryCity", label: "Citta principale" },
  { id: "interestLocations", label: "Localita d'interesse" },
  ...
  { id: "accessSource", label: "Tipo di attivazione" },
  { id: "primeStartedAt", label: "Data attivazione PRIME" },
  { id: "renewalOrExpiryAt", label: "Prossimo rinnovo o scadenza" },
  { id: "cancelAtPeriodEnd", label: "Disdetta programmata" },
  { id: "canceledAt", label: "Data disdetta" },
  { id: "totalPaid", label: "Totale pagato" },
  { id: "paymentCount", label: "Numero pagamenti" },
  { id: "lastPaymentAt", label: "Ultimo pagamento" },
];
...
export function PrimeSubscriberExportModal({
  open,
  isSuperAdmin,
  actorId,
  search,
  ...
  subscriberManagerId: string;
  getAccessToken: () => Promise<string | null | undefined>;
  onClose: () => void;
}) {
  const storageKey = useMemo(
    () => `leadhost:prime-subscriber-export-columns:${actorId ?? "super-admin"}`,
    [actorId],
  );
  const [scope, setScope] = useState<"all" | "filtered">("all");
  const [selectedColumns, setSelectedColumns] = useState<PrimeSubscriberExportColumn[]>(() => {
    ...
    try {
      ...
      const valid = Array.isArray(parsed)
        ? parsed.filter((value): value is PrimeSubscriberExportColumn =>
  ...
  const hasActiveFilters = Boolean(
    search ||
      ...
      (isSuperAdmin && subscriberManagerId),
  );

  function toggleColumn(column: PrimeSubscriberExportColumn) {
  ...
  async function download() {
    ...
    try {
      ...
      const response = await fetch("/api/admin/prime/export", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          managedProperties:
            scope === "filtered" && managedProperties ? managedProperties : undefined,
          subscriberStatus: scope === "filtered" ? subscriberStatus : "all",
          subscriberManagerId:
            scope === "filtered" && isSuperAdmin && subscriberManagerId
              ? subscriberManagerId
              : undefined,
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.error ?? "Non riesco a generare l'esportazione PRIME.");
      }
      ...
      anchor.href = url;
      anchor.download = "lead-host-abbonati-prime.csv";
      ...
      setError(
        downloadError instanceof Error
          ...
          : "Non riesco a generare l'esportazione PRIME.",
      );
    } finally {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="prime-subscriber-export-title"
    >
      <div className="max-h-[92dvh] w-full overflow-y-auto rounded-t-xl bg-white shadow-2xl sm:max-w-3xl sm:rounded-xl">
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4 sm:px-6">
          <div>
            ...
            <h2 id="prime-subscriber-export-title" className="mt-1 text-xl font-semibold text-ink">
              Esporta Abbonati PRIME
            </h2>
          </div>
          <button
            type="button"
          ...
          <section>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 p-4 transition has-[:checked]:border-green has-[:checked]:bg-green/5">
                <input
                  type="radio"
                  name="prime-subscriber-export-scope"
                ...
                <span>
                  ...
                  <span className="mt-1 block text-sm text-muted">
                    {isSuperAdmin
                      ? "Esporta l'intero elenco degli abbonati."
                      : "Esporta esclusivamente i tuoi PM PRIME."}
                  </span>
                </span>
              </label>
              <label
              ...
              >
                ...
                <span>
                  ...
                  <span className="mt-1 block text-sm text-muted">
                    {hasActiveFilters
                      ? "Usa i filtri applicati nella tab Abbonati PRIME."
                      : "Applica almeno un filtro per usare questa opzione."}
                  </span>
                </span>
              </label>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  I dati di fatturazione e le note interne non vengono esportati.
                </p>
              </div>
              <button
                type="button"
        ...
        <div className="sticky bottom-0 flex flex-col-reverse gap-3 border-t border-slate-200 bg-white p-5 sm:flex-row sm:justify-end sm:px-6">
          ...
          <button
            ...
            disabled={isDownloading || !selectedColumns.length}
          >
